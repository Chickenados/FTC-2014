...
#pragma config(Motor,  motorC,          gripperServo,  tmotorNXT, openLoop, encoder)
...
void gripperMovement(int close, int open){    //gripper programing, button a is 2, b is 3
	if(joy2Btn(2) == 1 && joy2Btn(3) == 0){
		servo[gripperServo] = servo[gripperServo] + GRIPPERGRABRATE;
	}
...
task main(){
	...
	while(true){
		...
		int joy1y1 = joystick.joy1_y1; int joy1y2 = joystick.joy1_y2; int joy1x1 = joystick.joy1_x1; int joy1x2 = joystick.joy1_x2; // Makes joystick positions easier to use
		int joy2y1 = joystick.joy2_y1; int joy2y2 = joystick.joy2_y1; int joy2x1 = joystick.joy2_x1; int joy2x2 = joystick.joy2_x2;
		movement(joy1x1, joy1y1, joy1x2, joy1y2); // Pass the joystick coordinates to movement method
		armMovement(joy2x1, joy2y1, joy2x2, joy2y2);
